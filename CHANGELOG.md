...
- **`build_pending_composites()`** — build the declared-but-not-yet-built composites. Called for you at the end of discovery; exposed because it is part of the discovery contract. (#147)
...
- **`discover_algorithms_in_package()` registers cfspopcon's own algorithms first**, so a composite declared by the walked package can be built from them, and a registry query made by a module during the walk no longer builds composites early. (#147)
...
- **`CompositeAlgorithm.from_list`** — to build an unnamed composite from a list of names, index the registry instead: `algorithms[["a", "b"]]`. To register a named one, use `CompositeAlgorithm.register_from_list`; the `name` and `register` arguments have no direct replacement. (#147)
