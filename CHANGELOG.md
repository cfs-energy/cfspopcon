...
- **A `plugins:` section in `input.yaml`** — a list of importable packages, registered before the file's `algorithms` names are resolved, so a case may name algorithms and variables from a package outside cfspopcon. The `popcon` command needs no extra flag. Note that this makes an input file able to import arbitrary modules, which is code execution driven by a data file.
- **`register_plugin()` / `register_plugins()`** — register external plugin packages, which register their own algorithms, variables and units, with clash detection, rollback and a report of what each added. Registering several together is what lets a composite in one plugin name an algorithm from another. (#148)
...
- **`extend_default_units_map`, `default_units_map` and `check_units_are_valid` accept `str | None`** — a `None` unit marks a variable which is not a unitful quantity and is passed through unconverted, which 18 of cfspopcon's own variables already relied on. `check_units_are_valid` now skips `None` rather than letting pint coerce it to dimensionless.
- **`reset_default_units()` empties the map completely**, cfspopcon's own entries included; it does not reload them. Its docstring now says so.
