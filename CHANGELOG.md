...
- **The label decorator is `@declare_algorithm(return_keys=...)`** (**breaking**): a module-level decorator, exported from `cfspopcon`, replacing `@Algorithm.register_algorithm`. Update decorated functions in place.
