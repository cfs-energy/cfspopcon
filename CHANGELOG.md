...
- **`discover_algorithms_in_package(package)`** — walk any package to register the algorithms defined beneath it. This is how a code built on cfspopcon adds algorithms of its own: call `discover_builtin_algorithms()` and then this, from that code's own command line or script, before reading an input file. (#147)
- **`discover_builtin_algorithms()`** — populate the registry by walking `cfspopcon.formulas`. Importing cfspopcon registers nothing, so call this before using the registry. Repeated calls change nothing. (#147)
