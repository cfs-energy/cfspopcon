...
- **`discover_builtin_algorithms()`** — populate the registry now rather than on the first read, to front-load the cost, fail early on a broken installation, or pick up a module added since discovery last ran. It runs the same discovery a read would, so entry-point providers load too. (#147)
...
- **The algorithm registry is populated lazily** — discovery runs once, on the first read of it, whether that is through `Algorithm.get_algorithm()`, `Algorithm.algorithms()`, `Algorithm.write_yaml()`, the `cfspopcon.registry` accessor, or `Algorithm.instances` itself. Reading `Algorithm.instances` completes discovery first, so it cannot quietly return a partially-registered registry. (#147)
