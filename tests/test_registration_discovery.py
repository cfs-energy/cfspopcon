...
the "registered-but-not-importable" gap it guarded against can no longer occur. The second half
covers a package built on cfspopcon, which extends the registry by walking itself.
"""
...
def test_the_registry_is_empty_until_discovery_runs(run_script):
    ...
    An empty registry reads as "nothing discovered yet"; one holding whatever happened to be imported
    would look like a real registry missing algorithms. Run in a subprocess, since the suite
    discovers at session start.
    """
    script = (
        "import cfspopcon\n"
...
def test_formulas_submodule_resolves_without_discovery(run_script):
    ...
    Run in a subprocess: discovery binds every submodule as a real attribute, so __getattr__ would
    never be consulted once the suite's session fixture has run.
    """
    script = (
        "import cfspopcon\n"
...
def test_a_package_name_which_does_not_resolve_leaves_the_registry_usable():
    ...
    with pytest.raises(ModuleNotFoundError):
        discover_algorithms_in_package("_probe_no_such_package")
...
def test_the_popcon_command_discovers_before_reading_the_case(run_script):
    ...
    Stops at read_case rather than running the case, since only the ordering is under test, and runs
    in a subprocess because the suite has already discovered in this one.
    """
    script = (
...
def test_a_walk_extends_the_builtin_registry(downstream_package):
    ...
    member_names = [alg._name for alg in Algorithm.get_algorithm("_ds_deep").algorithms]
