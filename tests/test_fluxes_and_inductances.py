...
def test_functionals_against_hirshman():
    ...
    assert np.allclose(
        fa, np.array([2.975, 2.285, 1.848, 1.507, 1.217, 0.957, 0.716, 0.487, 0.272]), rtol=5e-2
    )
    assert np.allclose(
        fb, np.array([0.228, 0.325, 0.403, 0.465, 0.512, 0.542, 0.553, 0.538, 0.508]), rtol=5e-2
    )
    assert np.allclose(
        fc, np.array([1.008, 1.038, 1.093, 1.177, 1.301, 1.486, 1.769, 2.223, 2.864]), rtol=5e-2
    )
    assert np.allclose(
        fd, np.array([0.022, 0.056, 0.087, 0.113, 0.134, 0.148, 0.155, 0.152, 0.133]), rtol=2.2e-1
    )
...
def test_inductances_against_hirshman():
    ...
    normalized_external_inductance = (calc_external_inductance(1 / A, kappa, 0, R0, 0, coeffs) / (constants.mu_0 * A)).data.to('henry').magnitude

    assert all(
        np.isclose(
            vertical_mutual_inductance, np.array([0.99, 0.96, 0.88, 0.83, 0.74, 0.52]), rtol=2e-2
        )
    )
    assert all(
        np.isclose(
