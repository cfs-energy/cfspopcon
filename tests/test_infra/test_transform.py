...
from cfspopcon import transform
...
def test_order_dimensions(z):
    ...
    with pytest.raises(ValueError):
        assert transform.order_dimensions(z.isel(x=0), dims=("x", "y")).dims == ("x", "y")

    assert transform.order_dimensions(z.isel(x=0), dims=("x", "y"), template=z).dims == ("y", "x")
    ds = xr.Dataset(dict(z=z))
    assert transform.order_dimensions(z.isel(x=0), dims=("x", "y"), template=ds).dims == ("y", "x")
