...
def process_input_dictionary(
    ...
    Several processing steps are applied, including;
        * The `plugins` entry, if present, is a list of importable packages, registered with `cfspopcon.register_plugins` before anything else, so that the rest of the file may name their algorithms and variables.
...
def process_plugins(repr_d: dict[str, Any]) -> None:
    """Register the plugin packages the input file lists, so that it can name their algorithms.

    The whole set is registered in one call, and before the ``algorithms`` names are resolved and the
    remaining keys have their units looked up, since a plugin supplies both.

    Note that this imports the modules an input file names: a case file from an untrusted source will
    run its plugins' module-level code. Pass plugins on the command line instead if that matters.
    """
    ...
    try:
        ...
        raise ModuleNotFoundError(
            f"Could not import '{exc.name}', listed in the 'plugins' section of the input file. A plugin must be an "
            "importable package; note that the import name may differ from the name of the distribution providing it."
        ) from exc
