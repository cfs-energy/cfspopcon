...
class AtomicData:
    """
    A class to manage atomic data for various species, providing facilities for accessing datasets directly or
    by constructing interpolators for different datasets (i.e. radiated power curves)
    ...
    @staticmethod
    def read_atomic_data(atomic_data_directory: Path = Path() / "radas_dir"):
        """
        ...
        if not (atomic_data_directory / "output").exists():
            raise FileNotFoundError(f"atomic_data_directory ({atomic_data_directory}) does not contain a subfolder called 'output'. Make sure you have executed `poetry run radas` before calling this function.")
    ...
    def __getitem__(self, species: Union[str, AtomicSpecies]):
        """
    ...
    def get_dataset(self, species: Union[str, AtomicSpecies]):
        """
    ...
    def get_interpolator(self, kind: int, species: Union[str, AtomicSpecies], ne_tau: float=np.inf):
        """
        ...
        if not species in self.available_species:
            raise FileNotFoundError(f"Requested data for {species.name} but no corresponding {species.name.lower()}.nc dataset is available in {(self.atomic_data_directory / 'output').absolute()}")
        ...
        if (ne_tau < np.inf) and (ne_tau not in dataset.dim_ne_tau):
            raise RuntimeError(f"Requested a value of ne_tau ({ne_tau} m^-3 s) which was not in the available values ({list(dataset.dim_ne_tau.values)}). Check the documentation on configuring radas.")
        if ((kind in [self.CoronalLz, self.CoronalZ]) and (ne_tau < np.inf)):
            raise RuntimeError(f"Requested coronal data. ne_tau will have no effect.")
    ...
    def eval_interpolator(self,
                          electron_density: xr.DataArray, electron_temp: xr.DataArray,
                          kind: int, species: Union[str, AtomicSpecies], ne_tau: float=np.inf,
                          allow_extrapolation: bool=False, grid: bool=True,
                          ) -> xr.DataArray:
        """
        ...
        coords = dict(dim_electron_temp=electron_temp, dim_electron_density=electron_density)  # Prepare coordinates for the result DataArray
        ...
        if allow_extrapolation:
            ...
            assert electron_temp.max() <= dataset.dim_electron_temp.max(), f"{electron_temp.max()} > {dataset.dim_electron_temp.max().item()}"
            assert electron_temp.min() >= dataset.dim_electron_temp.min(), f"{electron_temp.min()} < {dataset.dim_electron_temp.min().item()}"
            assert electron_density.max() <= dataset.dim_electron_density.max(), f"{electron_density.max()} > {dataset.dim_electron_density.max().item()}"
            assert electron_density.min() >= dataset.dim_electron_density.min(), f"{electron_density.min()} < {dataset.dim_electron_density.min().item()}"
