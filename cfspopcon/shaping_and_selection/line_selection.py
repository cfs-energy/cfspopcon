...
def find_coords_of_contour(
    array: xr.DataArray, x_coord: str, y_coord: str, level: Quantity
    ...
    contour_x = xr.DataArray(
        lines[:, 1], coords={x_coord: lines[:, 1]}
    )  # type:ignore[call-overload]
    contour_y = xr.DataArray(
        lines[:, 0], coords={x_coord: lines[:, 1]}
    )  # type:ignore[call-overload]
...
def interpolate_onto_line(
    ...
    if not array.ndim == 2:
        raise RuntimeError(
            "Contour interpolation only supported for 2 dimensions. Use apply_ufunc to apply to higher dimensions."
        )
    ...
    return xr.DataArray(interpolated_values, coords={x_coord: line_x}).pint.quantify(
        units
