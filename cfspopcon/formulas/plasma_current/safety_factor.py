...
@declare_algorithm(return_keys=["f_shaping"])
...
@declare_algorithm(return_keys=["plasma_current"])
@wraps_ufunc(
    input_units=dict(
...
@declare_algorithm(return_keys=["q_star"])
@wraps_ufunc(
    input_units=dict(
        magnetic_field_on_axis=ureg.T,
        major_radius=ureg.m,
...
@declare_algorithm(return_keys=["cylindrical_safety_factor"])
