...
from ...algorithm_class import Algorithm, CompositeAlgorithm, declare_algorithm
...
@declare_algorithm(return_keys=["rho"])
def define_radial_grid(n_points_for_confined_region_profiles: int = 50) -> np.ndarray:
    """Define the radial grid for profiles.

    The grid is nudged just inside the LCFS (see :func:`build_rho_grid`) rather
    than dropping the endpoint, so the last point stays essentially at rho = 1
    while the analytic hollow-profile form remains finite at the edge.
    """
    x = build_rho_grid(n_points_for_confined_region_profiles)
    return xr.DataArray(x, coords=dict(dim_rho=x))


@declare_algorithm(return_keys=["electron_density_profile", "fuel_ion_density_profile", "electron_temp_profile", "ion_temp_profile"])
