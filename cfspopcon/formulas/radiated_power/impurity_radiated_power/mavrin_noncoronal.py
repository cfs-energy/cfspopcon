...
def calc_impurity_radiated_power_mavrin_noncoronal(  # noqa: PLR0912
    ...
    if impurity_Z not in zimp:  # pragma: no cover
        warnings.warn(
            f"Mavrin 2017 line radiation calculation not supported for impurity with Z={impurity_Z}",
            stacklevel=3,
        )
        return np.nan
    ...
    if impurity_Z == 2:  # Helium
        ...
        temperature_bin_borders = np.array(
            [1.0, 7.0, 30.0, 60.0, 100.0, 1000.0, 10000.0]
        )
        radc = np.array(
            [
                [
        ...
        temperature_bin_borders = np.array(
            [1.0, 7.0, 20.0, 70.0, 200.0, 700.0, 15000.0]
        )
        radc = np.array(
            [
                [
        ...
        temperature_bin_borders = np.array(
            [1.0, 10.0, 30.0, 100.0, 300.0, 1000.0, 15000.0]
        )
        radc = np.array(
            [
                [
        ...
        temperature_bin_borders = np.array(
            [1.0, 10.0, 70.0, 300.0, 1000.0, 3000.0, 15000.0]
        )
        radc = np.array(
            [
                [
        ...
        temperature_bin_borders = np.array(
            [1.0, 10.0, 50.0, 150.0, 500.0, 1500.0, 10000.0]
        )
        radc = np.array(
            [
        ...
        raise RuntimeError(
            "This should never happen, please ensure all impurity cases in zimp array are covered!"
        )
    ...
    electron_temp_profile = np.maximum(
        electron_temp_profile, temperature_bin_borders[0]
    )
    electron_temp_profile = np.minimum(
        electron_temp_profile, temperature_bin_borders[-1]
    )
    ...
    if np.any(Y_vals > 0.0):  # pragma: no cover
        warnings.warn(
            "Warning: treating points with ne_tau_i_per_m3 > 1e19 m^-3 s as coronal.",
            stacklevel=3,
        )
