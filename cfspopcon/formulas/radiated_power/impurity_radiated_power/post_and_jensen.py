...
def calc_impurity_radiated_power_post_and_jensen(
    ...
    if impurity_Z not in zimp:  # pragma: no cover
        warnings.warn(
            f"Post & Jenson radiation calculation not supported for impurity with Z={impurity_Z}",
            stacklevel=3,
        )
        return np.nan
    ...
    Tmin = np.array(
        [0.002, 0.002, 0.002, 0.003, 0.005, 0.03, 0.02, 0.02, 0.03, 0.06, 0.1]
    )

    # If trying to evaluate for a temperature outside of the given range, assume nearest neighbor
    # and throw a warning
    if any(electron_temp_profile < Tmin[iz]) or any(
        electron_temp_profile > 100
    ):  # pragma: no cover
        warnings.warn(
    ...
    for i in range(len(radc)):
        it = np.nonzero(
            (electron_temp_profile >= temperature_bin_borders[i])
            & (electron_temp_profile < temperature_bin_borders[i + 1])
        )[0]
        if it.size > 0:
            log10_Lz[it] = polyval(Tlog[it], radc[i, :, iz])  # type: ignore[no-untyped-call]
