...
class AtomicData:
    ...
    def __init__(self, atomic_data_directory: Path = Path() / "radas_dir") -> None:
        ...
        for species in self.available_species:
            ...
            for ne_tau, subds in dataset.groupby("dim_ne_tau"):
                subds = subds.squeeze(dim="dim_ne_tau")
    ...
    def get_noncoronal_Lz_interpolator(self, species: str | AtomicSpecies, ne_tau: float | Quantity) -> CoeffInterpolator:
        """Returns a noncoronal_Lz_interpolator for the specified species and ne_tau value."""
        if isinstance(ne_tau, Quantity):
            ne_tau = float(magnitude_in_units(ne_tau, ureg.m**-3 * ureg.s))
        return self.noncoronal_Lz_interpolators[(self.key_to_enum(species), ne_tau)]

    def get_noncoronal_Z_interpolator(self, species: str | AtomicSpecies, ne_tau: float | Quantity) -> CoeffInterpolator:
